...
First, a heatmap in order to compare the average of certain features for the artists chosen above. The main goal is to get an idea of how the playlists compare on average.

### Parallel coordinates [track-level features]
...
Next, we dive deeper and take a look at the parallel coordinates for each playlist

### Scatter plots [track-level features]
...
From the parallel coordinates above, we can see that acousticness and valence have the most variance. In order to further analyze this variance, we plot both of these for every track in the corpus against eachother in a scatterplot.
